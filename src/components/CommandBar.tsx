...
// ── Smart Italian/French query parser ─────────────────────────────────────────
...
function parseQuery(raw: string, plan: DailyPlan[]): CommandResult | null {
  ...
  if (words.some(w => ['oggi', 'aujourd', 'today'].includes(w))) {
    ...
    // Check for weekday name: "lunedì", "martedì" …
  ...
  // ── City / client name filter ─────────────────────────────────────────────
  // Skip common stopwords and already-processed keywords
  const STOPWORDS = new Set([
    'le', 'la', 'le', 'les', 'di', 'da', 'a', 'su', 'per', 'in', 'con',
    ...
    'cliente', 'client', 'clienti', ...Object.keys(DAY_NAMES),
  ])
...
const EXAMPLES = [
  'visite di oggi',
  'urgenti questa settimana',
  'urgenti domani',
  'visite a Charleroi',
  'clienti urgenti martedì',
]
...
export function CommandBar({ plan, onResult }: CommandBarProps) {
  ...
  const run = useCallback((q: string) => {
    const result = parseQuery(q, plan)
    onResult(result)
  }, [plan, onResult])
    ...
    <div className="w-full space-y-2">
      ...
      <div className={`relative flex items-center gap-2 bg-white dark:bg-slate-800 rounded-2xl border-2 transition-all duration-200 shadow-sm ${
        ...
        <input
          ...
          placeholder='Es: "urgenti oggi", "visite a Charleroi", "urgenti martedì"'
          ...
          >
            {EXAMPLES.map(ex => (
              <button
                ...
                className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs hover:bg-indigo-100 dark:hover:bg-indigo-900/40 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors"
              >
